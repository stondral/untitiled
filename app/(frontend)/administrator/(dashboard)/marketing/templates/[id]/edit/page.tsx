...
export default function EditTemplatePage() {
        ...
        fetch(`/api/email-templates/${id}`)
            ...
            .then(data => {
                setFormData({
                    ...
                    shellUnlocked: data.shellUnlocked || false
                });
            })
  ...
  const toggleStandard = (checked: boolean) => {
    setFormData(prev => {
        ...
        if (checked && !prev.shellUnlocked && !prev.body.includes("<html")) {
            // Apply shell if engaging standard and body doesn't look like a full HTML doc
  ...
  const insertVar = (v: string) => {
    const val = (formData.isStandard ? formData.plainContent : formData.body) + ` {${v}} `;
    updateBody(val);
  };
    ...
    <div className="max-w-5xl mx-auto py-8 px-4 space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tighter">Edit Outreach Template</h1>
            <p className="text-slate-500 font-bold text-sm uppercase tracking-widest">Refine your high-performance hook</p>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                  <button 
                      ...
                      className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${view === "edit" ? "bg-white dark:bg-slate-700 text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
                      <Edit className="w-4 h-4 inline mr-2" /> Design
                  </button>
                  <button 
                      onClick={() => setView("preview")}
                      className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${view === "preview" ? "bg-white dark:bg-slate-700 text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
            ...
            {view === "edit" ? (
              <form id="template-form" onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="p-6 bg-indigo-50 dark:bg-indigo-500/5 rounded-3xl border border-indigo-100 dark:border-indigo-500/10 flex items-center justify-between group">
                      <div className="flex items-center gap-4">
                          <div className={`h-12 w-12 rounded-2xl flex items-center justify-center transition-all ${formData.isStandard ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/30" : "bg-white dark:bg-slate-800 text-slate-400 border border-slate-200 dark:border-slate-700"}`}>
                              {formData.isStandard ? <Lock className="w-6 h-6" /> : <Edit className="w-6 h-6" />}
                          </div>
                          <div>
                      ...
                      <div className="flex items-center justify-between px-1">
                          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                              <button 
                                  ...
                                  className={`px-4 py-2 rounded-lg text-[10px] font-black transition-all ${editorTab === "content" ? "bg-white dark:bg-slate-700 text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                              >
                              ...
                              <button 
                                  ...
                                  className={`px-4 py-2 rounded-lg text-[10px] font-black transition-all ${editorTab === "html" ? "bg-white dark:bg-slate-700 text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                              >
        ...
        <div className="space-y-6">
          <Card className="p-8 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-[2.5rem]">
            ...
            <p className="text-sm text-slate-500 font-bold leading-relaxed mb-8">
              Updating this template will affect all future sends using this identifier.
            </p>
            <div className="space-y-3">
                ...
                >
                    {submitting ? <RefreshCw className="w-5 h-5 animate-spin" /> : <><Save className="w-5 h-5 mr-2" /> Save Update</>}
                </Button>
                <Link href="/administrator/marketing/templates">
